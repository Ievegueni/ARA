...
export function Sidebar(p: Props) {
  ...
  return (
    <>
      {p.open && <div className="fixed inset-0 z-30 bg-ink-950/50 backdrop-blur-sm lg:hidden" onClick={p.onClose} />}
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex w-72 flex-col bg-ink-950 text-ink-200 transition-transform duration-300 lg:static lg:translate-x-0 ${
          p.open ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="flex items-center justify-between px-5 pt-5 pb-4">
          <Logo dark />
          <button onClick={p.onClose} className="rounded-lg p-1.5 text-ink-400 hover:bg-white/5 lg:hidden" aria-label="Fechar menu">
            <X className="size-5" />
          </button>
        </div>
            ...
            {(
              ...
              <button
                ...
                className={`flex items-center justify-center gap-1.5 rounded-lg py-2 transition ${
                  p.view === v ? "bg-white/10 text-white" : "text-ink-400 hover:text-ink-200"
                }`}
              >
        ...
        <div className="scroll-dark mt-4 flex-1 overflow-y-auto px-3 pb-4">
          {p.conversations.length === 0 ? (
            <p className="px-3 py-6 text-center text-xs text-ink-500">As suas conversas aparecem aqui.</p>
          ) : (
            groupByDate(p.conversations).map(([label, items]) => (
              <div key={label} className="mb-4">
                <div className="px-3 pb-1.5 text-[11px] font-semibold tracking-wider text-ink-500 uppercase">{label}</div>
                {items.map((c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="mr-1.5 rounded-md p-1 text-ink-500 opacity-0 transition group-hover:opacity-100 hover:bg-white/10 hover:text-red-400 focus:opacity-100"
        ...
        <div className="flex items-center gap-3 border-t border-white/5 px-4 py-4">
          ...
          <div className="min-w-0 flex-1 leading-tight">
            ...
            <div className="truncate text-xs text-ink-500">{p.user.section ?? (p.user.role === "ADMIN" ? "Administrador" : "Técnico")}</div>
          </div>
          <button onClick={p.onLogout} className="rounded-lg p-2 text-ink-400 transition hover:bg-white/5 hover:text-white" aria-label="Sair" title="Sair">
